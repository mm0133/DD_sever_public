...
#이친구는 get_QuestionToManager에서 접근권한을 제한했음
class QuestionToManagerViewWithPk(APIView):
    ...
    def get_QuestionToManager(self,request, pk):
        try:
            ...
            if (not questionToManager.isPrivate) or (request.user== questionToManager.writer) or True:#True대신에 관리자 로직 넣을것
...
class CommentToQuestionViewWithPK(APIView):#댓글 수정삭제, get요청은 잘안쓸거같긴한데 나중에 혹시 ajax에서 쓸수있으니 구현해놈
    ...
    def get_QuestionToManager(self,request, pk):
        try:
            ...
            if (not commentToQuestion.isPrivate) or (request.user== commentToQuestion.writer) or True:#True대신에 관리자 로직 넣을것
...
class FeedbackToManagerViewWithPk(APIView):
    ...
    #         return Response(status=status.HTTP_401_UNAUTHORIZED)
    #
    #     serializer = FeedbackToManagerSerializer(feedbackToManager, data=request.data, partial=True)
    #     if serializer.is_valid():#validate 로직 추가
    #         feedbackToManager = serializer.save()
    #         return Response(FeedbackToManagerSerializer(feedbackToManager).data)
    #     else:
    #         return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
